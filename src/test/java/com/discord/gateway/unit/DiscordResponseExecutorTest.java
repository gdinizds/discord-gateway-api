...
class DiscordResponseExecutorTest {
    ...
    @BeforeEach
    void setUp() {
        executor = new DiscordResponseExecutor(hookRegistry);
    ...
    void replyCallsSendMessage() {
        ...
        var payload = new OutboundResponsePayload(
                "REPLY", "token123", null, null, "Hello!", null, null);
    ...
    void ephemeralReplyCallsSendMessageWithEphemeralFlag() {
        ...
        var payload = new OutboundResponsePayload(
                "EPHEMERAL_REPLY", "token123", null, null, "Only you!", null, null);
    ...
    void updateMessageCallsEditOriginal() {
        ...
        var payload = new OutboundResponsePayload(
                "UPDATE_MESSAGE", "token123", "msg123", null, "Edited!", null, null);
        executor.execute(payload);
    ...
    @Test
    @SuppressWarnings("unchecked")
    void deferredReplyCallsSendMessage() {
        // DEFERRED_REPLY sends content as followup to an already-acknowledged interaction
        var msgAction = mock(WebhookMessageCreateAction.class);
        when(hook.sendMessage(anyString())).thenReturn(msgAction);

        var payload = new OutboundResponsePayload(
                "DEFERRED_REPLY", "token123", null, null, "Processing done!", null, null);
        executor.execute(payload);

        verify(hook).sendMessage("Processing done!");
    }
    ...
    void deferredUpdateCallsEditOriginal() {
        ...
        var payload = new OutboundResponsePayload(
                "DEFERRED_UPDATE", "token123", "msg123", null, "Updated!", null, null);
        executor.execute(payload);
    ...
    void hookNotFoundReturnsFailureResult() {
        ...
        var payload = new OutboundResponsePayload(
                "REPLY", "missing-token", null, null, "Hello!", null, null);
        var result = executor.execute(payload);
